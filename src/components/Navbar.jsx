...
const Navbar = () => {
  ...
  const navLinks = [
    { path: '/', label: 'Home', category: null },
    { path: '/', label: 'Shop All', category: 'All' },
    { path: '/?category=Hoodies', label: 'Hoodies', category: 'Hoodies' },
    { path: '/?category=T-Shirts', label: 'T-Shirts', category: 'T-Shirts' },
    { path: '/?category=Sweatshirts', label: 'Sweatshirts', category: 'Sweatshirts' },
    { path: '/?category=Co-ord Sets', label: 'Sets', category: 'Co-ord Sets' },
  ];

  const isActive = (link) => {
    // Home: active only on root path with no category filter
    if (link.category === null) {
      return location.pathname === '/' && !currentCategory;
    }

    // Shop All: active when on home with no specific category param
    if (link.category === 'All') {
      return location.pathname === '/' && !currentCategory;
    }

    // Category links: active when the URL category matches
